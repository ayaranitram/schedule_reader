...
def extract_keyword(schedule_dict:dict, keyword: str = None, record_names=None) -> pd.DataFrame:
    """
    from the provided schedule dictionary `schedule_dict` extract the desired `keyword`, create a DataFrame and set the column names as the `record_names` provided (optional).
    ...
    # look for specified keyword, only keep the last previous date
